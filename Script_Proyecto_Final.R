...
#RESPUESTA: 
#El endeudamiento del activo fue mayor en las pequeñas + las microempresas en comparación de las grandes empresas. 
...
#sumar fila 1;3 y 4 
NoDefinida <- endeudamientoactivo_promedio_tipoempresa[4,]
nueva <- endeudamientoactivo_promedio_tipoempresa[c(3,5),]
Grande <- endeudamientoactivo_promedio_tipoempresa[1,]

peq_micro <- nueva %>% summarise(across(where(is.numeric), sum, na.rm = TRUE))

peq_micro$Tamanio <- "Peq_micro"

peq_micro$endeudamientoactivo_prom <- sum(peq_micro$endeudamientoactivo_prom,na.rm=TRUE)

comparativo_empresas <- rbind(peq_micro,Grande)
